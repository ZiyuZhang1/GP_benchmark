...
def one_fold_evaluate(disease, time, feature_list, df, y, train_idx, test_idx, methods, result_df, fold):
    ...
    if 'early_fusion' in methods:
        print('early fusion')
        bagging_y_scores = [neg_bagging_early(args) for args in args_list]
    ...
    if 'mid_fusion' in methods:
        print('mid fusion')
        bagging_y_scores = [neg_bagging_mid(args) for args in args_list]
        all_preds, all_aucs = zip(*bagging_y_scores)
        final_y_score = mask_mean(all_preds)
        mean_auc = np.mean(all_aucs)
        print('mid fusion validation auc:', mean_auc)
        ranked_predict_index, results = eval_bagging(final_y_score, y_test)
    ...
    if 'later_fusion' in methods:
        print('later fusion')
        bagging_y_scores = [neg_bagging_later(args) for args in args_list]
